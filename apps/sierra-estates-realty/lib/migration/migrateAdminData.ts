...
export async function createMigrationAuditTrail(
  ...
  try {
    await addDoc(collection(db, 'activities'), {
      ...
      changes: {
        totalCollectionsMigrated: results.reduce((sum: number, r) => sum + r.migratedCollections.length, 0 as number),
        totalRecordsMigrated: results.reduce((sum: number, r) => sum + r.recordsMigrated, 0 as number),
        totalErrors: results.reduce((sum: number, r) => sum + r.errors.length, 0 as number),
